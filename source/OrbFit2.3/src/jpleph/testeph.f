...
C     user can extract those subroutines and the installation process is complet
C                                                                               
...
C         record.  For some compilers, it is given in bytes; in some, it is give
...
C         varies from one JPL ephemeris to another, RECL must be determined some
...
C             not in UNIX.                                                      
C                                                                               
C         b)  Open the file with an arbitrary value of RECL, read the first reco
C             and use the information on that record to determine the exact valu
C             of RECL.  Then, close the file and re-open it with the exact value
C             This seems to work for UNIX compilers as long as the initial value
C             RECL is less than the exact value but large enough to get the requ
C             information from the first file.  (For other compilers, this doesn
...
C  sometimes, an error return statement number; sometimes, a logical denoting   
C  whether or not the requested date is covered by the ephemeris.  We apologize 
C  for this inconsistency; in this present version, we use only the four necessa
C  arguments and do the testing outside of the subroutine.                      
C                                                                               
C                                                                               
C                                                                               
