...
! interface with twobodyint
  TYPE(orbit_elem), DIMENSION(nprelx) :: elpr
  TYPE(orb_uncert) :: orbunc(nprelx) !covariance
  REAL(KIND=8) :: deltaoml(nprelx,2) 
  REAL(KIND=8) :: norm(nprelx) ! identification norm
  INTEGER :: nsol_step(4) 
  INTEGER nprel,iprel
  DOUBLE PRECISION, DIMENSION(3) :: qobs1,qpobs1,qppobs1 
  DOUBLE PRECISION, DIMENSION(3) :: qobs2,qpobs2,qppobs2 
...
! coordinate changes
     ...
     menunam='cooarc' 
     IF(cooy.eq.'ATT')THEN
        WRITE(*,*)' observatory code integer, 500=geocentric '
        READ(*,*) stacod
     ENDIF
     ...
     IF(iarc.eq.1.or.iarc.eq.3)THEN
        IF(ini0)THEN
           IF(cov0)THEN
              CALL coo_cha(el0,cooy,el0,fail_flag,dee,OBSCODE=stacod)
              CALL convertunc(unc0,dee,unc0)
           ELSE
              CALL coo_cha(el0,cooy,el0,fail_flag,OBSCODE=stacod)
              unc0%succ=.false.
           ENDIF
           CALL write_elems(el0,astna0,'ML',dummyfile,iunel0,unc0)
           WRITE(*,*)' elements for arc 1', el0
           IF(cooy.eq.'ATT') Write(*,*)'tobs=',el0%t+el0%coord(5)/vlight
        ELSE
     ...
     IF(iarc.eq.2.or.iarc.eq.3)THEN
        IF(inip)THEN
           IF(covp)THEN
              CALL coo_cha(elp,cooy,elp,fail_flag,dee,OBSCODE=stacod)
              CALL convertunc(uncp,dee,uncp)
           ELSE
              CALL coo_cha(elp,cooy,elp,fail_flag,OBSCODE=stacod)
              uncp%succ=.false.
           ENDIF
           CALL write_elems(elp,astnap,'ML',dummyfile,iunelp,uncp)
           WRITE(*,*)' elements for arc 2', elp
           IF(cooy.eq.'ATT') Write(*,*)'tobs=',elp%t+elp%coord(5)/vlight
        ELSE
     ...
     IF(iarc.eq.4)THEN
        IF(initwo)THEN
           IF(covtwo)THEN
              CALL coo_cha(el,cooy,el,fail_flag,dee,OBSCODE=stacod)
              CALL convertunc(unc,dee,unc)
           ELSE
              CALL coo_cha(el,cooy,el,fail_flag,OBSCODE=stacod)
              unc%succ=.false.
           ENDIF
           CALL write_elems(el,astna0,'ML',dummyfile,iunelt,unc)
           WRITE(*,*)' elements for both arcs', el
           IF(cooy.eq.'ATT') Write(*,*)'tobs=',el%t+el%coord(5)/vlight
        ELSE
...
59   CALL tee(iun_log,'COMPUTE ATTRIBUTABLES=')
...
!        write(*,*)'siamo qui'
        CALL obs_cop(1,1) ! copy observations to obsc, obswc
        CALL attri_comp(mc,obsc,obswc,attr0,error,qobs1,qpobs1,qppobs1)
        IF(error) GOTO 50
        trou=nint(attr0%tdtobs)
        WRITE(*,*)' attributable of ',astna0
        IF(attr0%sph*radeg.gt.sphx)THEN
           WRITE(*,*)' arc too wide ', attr0%sph*radeg
        ELSE
           CALL wri_attri(0,0,astnac,attr0,trou)
        ENDIF
        CALL obs_cop(1,2) ! copy observations to obsc, obswc
        CALL attri_comp(mc,obsc,obswc,attrp,error,qobs2,qpobs2,qppobs2)
        IF(error) GOTO 50
        trou=nint(attrp%tdtobs)
        WRITE(*,*)' attributable of ',astnap
        IF(attrp%sph*radeg.gt.sphx)THEN
           WRITE(*,*)' arc too wide ', attrp%sph*radeg
        ELSE
           CALL wri_attri(0,0,astnac,attrp,trou)
        ENDIF
     ENDIF
     IF(iarc.lt.4)THEN
        menunam='dummy' 
...
177        FORMAT('nobs=',I4,' nights=',i3,' errcode=',i4/         &
        ...
        ENDIF
     ELSEIF(iarc.eq.4)THEN
        WRITE(*,*)' attributables from both arcs can be used for linkage 1=yes 0=no '
        READ(*,*) iatt
        IF(iatt.eq.0)THEN
           GOTO 50
        ELSE
           CALL tee(iun_log,'IDENTIFICATION BY INTEGRALS=')
           maxnorm=10.d0
!           DO j=1,300 !test of performance
              CALL twobodyint(attr0,attrp,maxnorm,nprel,elpr,&
                   &qobs1,qpobs1,qobs2,qpobs2,&
                   &orbunc,deltaoml,norm,nsol_step,CHECKDER=.false.)
!           ENDDO
!           STOP
! ingest preliminary orbit as computed; it is a guess for identification
! for now it is assigned to arc 2;
           DO j=1,nprel
              WRITE(*,790)j,norm(j),elpr(j)%coord
790           FORMAT('sol=',I2,' chi=',1P,D10.3,6(1X,D14.7))
              CALL write_elems(elpr(j),astna0,'ML',dummyfile,iunelt,orbunc(j))
           ENDDO
778        WRITE(*,*)nprel,' preliminary orbits available, which one?'
           READ(*,*) iprel
           IF(iprel.gt.0.and.iprel.le.nprel)THEN 
              el=elpr(iprel)
              initwo=.true.
           ELSE
              GOTO 778
           ENDIF
        ENDIF
     ELSEIF(iarc.gt.4)THEN
        WRITE(*,*)iarc,iatt, ' option not operational'
     ENDIF
! ===================================================================== 
