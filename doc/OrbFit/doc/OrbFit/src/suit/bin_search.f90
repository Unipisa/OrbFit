SUBROUTINE bin_search_rea(a,ntot,i1,i2,alist,ordind,indnum)
  ...
  INTEGER,INTENT(IN) :: ntot,i1,i2 ! number of elements in the list, pointer to the first and last to be considered
  ...
  INTEGER, INTENT(OUT) :: indnum ! address of the maximum of weak minorants in the list
     ...
     IF(nlist.eq.1) THEN
        IF(fst.eq.i2)THEN
           IF(a.ge.alist(ordind(fst)))THEN
              ...
              EXIT
           ELSE
!              WRITE(*,*) ' bin_search_rea: ', a,' not above ', alist(ordind(fst))
              indnum=fst-1 ! number is just before 
           ENDIF
        ELSEIF(a.ge.alist(ordind(fst)).and.a.lt.alist(ordind(fst+1)))THEN
           indnum=fst
        ELSE
!           WRITE(*,*) ' bin_search_rea: ', a,' not between ', alist(ordind(fst)), alist(ordind(fst+1))
           indnum=fst-1 ! number is just before 
        ENDIF   
        EXIT
     ENDIF
     nmez=nlist/2
!     IF(2*nmez.eq.nlist) THEN ! nlist even
        ...
        ELSE
           ...
           nlist=nlist-nmez        
        ENDIF
!     ELSEIF(2*nmez.lt.nlist) THEN ! nlist odd
!        IF(a.lt.alist(ordind(fst+nmez+1))) THEN
!           fst=fst
!           nlist=nmez+1 
!        ELSE
!           fst=fst+nmez+1
...
!     ENDIF
  ENDDO
!  WRITE(*,*)' bin_search_rea: nlist, no iter ',i2-i1+1,i
...
!     IF(2*nmez.eq.nlist) THEN ! nlist even
        ...
        ENDIF
!     ELSEIF(2*nmez.lt.nlist) THEN ! nlist odd
!        IF(LLT(name,namlist(ordind(fst+nmez+1)))) THEN
!           fst=fst
!           nlist=nmez+1 
!        ELSE
!           fst=fst+nmez+1
...
!     ENDIF
  ENDDO
!  WRITE(*,*)' bin_search: ntot, no iter ',ntot,i
...
! HEAPSORTNAINT                                                            
! sorting by heapsort of a name array of lenght n 
! by using numeric order        
! the input array a is not changed, in output                           
! ind is the indirect address of the sorted array                       
! ===================================                                   
SUBROUTINE heapsortnaint(a,n,aint,ind) 
  USE name_rules
  IMPLICIT NONE
  INTEGER,INTENT(IN) ::  n 
  CHARACTER(name_len),INTENT(IN) :: a(n)
  INTEGER, INTENT(OUT) :: aint(n)
  INTEGER, INTENT(OUT) :: ind(n) 
! =====end interface========                                            
  integer j,l,ir,indt,i, qi
!  CHARACTER(name_len) q
  DOUBLE PRECISION tq
! initialise indexes to natural order, convert to integer
! first 4 characters
  DO j=1,n 
     ind(j)=j 
     CALL decode_number (a(j)(1:4), aint(j))
  ENDDO
  IF(n.eq.1)RETURN 
! counters for recursion, length of heap                                
  l=n/2+1 
...
2 IF(j.le.ir)THEN 
     IF(j.lt.ir)THEN
        IF(aint(ind(j)).lt.aint(ind(j+1)))j=j+1
!        IF(LLT(a(ind(j)),a(ind(j+1))))j=j+1 
     ENDIF
     IF(qi.lt.aint(ind(j)))THEN
!     IF(LLT(q,a(ind(j))))THEN 
        ind(i)=ind(j) 
        i=j 
        j=j+j 
...
!     IF(2*nmez.eq.nlist) THEN ! nlist even
        ...
        ENDIF
!     ELSEIF(2*nmez.lt.nlist) THEN ! nlist odd
!        IF(iname.lt.inamlist(ordind(fst+nmez+1))) THEN
!           fst=fst
!           nlist=nmez+1 
!        ELSE
!           fst=fst+nmez+1
...
!     ENDIF
   ENDDO
!   WRITE(*,*)' bin_search_int: ntot, no iter ',ntot,i
