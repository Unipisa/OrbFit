...
!============================================================          
      ...
      IMPLICIT NONE 
!     input data                                                        
      DOUBLE PRECISION csi0,zeta0,sigma0,w0,s0,r0 
...
!     perturbing function                                               
      CALL dqags(ff,rmin,rmax,epsabs,epsrel,rm,abserr,neval,ier,limit   &
...
!     integrale                                                         
      call dqagsc(f,fstext,sndext,epsabs,epsrel,result,abserr,neval,ier,&
...
!     ===========================================================       
      DOUBLE PRECISION FUNCTION f(uu)
      USE fund_const 
      IMPLICIT NONE 
!     uu variable                                                       
      double precision uu 
...
!     FUNZIONE INTEGRANDA                                               
      f = (1.d0/(2.d0*pig*s*w))*exp(espo1 + espo2) 
...
! velocity at infinity, H magnitude, probability, difference in time)   
      DOUBLE PRECISION U, hmagn, prob, dt 
! output warnign file                                                   
      INTEGER iunwarn 
! OUTPUT                                                                
! mass, impact velocity, energy, expected energy                        
      DOUBLE PRECISION mass,v_imp,energy,e_tilde 
! frequency of impacts with given energy, relative probability          
      DOUBLE PRECISION fb, rel_prob 
! ==================end interface===========================            
...
! mass in kg; 1329 is a mistery number, where is it from? reference need
