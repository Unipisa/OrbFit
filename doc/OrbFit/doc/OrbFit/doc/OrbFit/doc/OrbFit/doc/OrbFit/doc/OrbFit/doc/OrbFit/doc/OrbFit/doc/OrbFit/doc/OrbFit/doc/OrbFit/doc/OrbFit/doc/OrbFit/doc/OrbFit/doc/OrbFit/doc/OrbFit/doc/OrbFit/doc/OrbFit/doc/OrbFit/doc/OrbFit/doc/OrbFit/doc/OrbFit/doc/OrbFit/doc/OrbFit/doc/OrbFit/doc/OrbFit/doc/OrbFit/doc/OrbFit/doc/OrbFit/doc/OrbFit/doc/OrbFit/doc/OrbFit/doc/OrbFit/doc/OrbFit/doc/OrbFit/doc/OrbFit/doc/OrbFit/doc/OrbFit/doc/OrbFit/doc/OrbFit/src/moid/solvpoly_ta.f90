...
! with real coefficients
 SUBROUTINE solvpoly(poldg,coef,roots,nroots,hzflag,multfl)
...
! *************************
   IF(abs(poly(0)).le.epsil) THEN
      IF(verb_moid.ge.20) THEN
         WRITE(*,*)'CONSTANT TERM is CLOSE to ZERO!',poly(0)
      ENDIF
      numzerosol = 1
      pdeg = pdeg-1
      IF(abs(poly(1)).le.epsil) THEN
         IF(verb_moid.ge.20) THEN
            WRITE(*,*)'FIRST DEGREE TERM is CLOSE to ZERO!',poly(1)
         ENDIF
         numzerosol = 2
         pdeg = pdeg-1
         IF(abs(poly(2)).le.epsil) THEN
            IF(verb_moid.ge.20) THEN
               WRITE(*,*)'SECOND DEGREE TERM is CLOSE to ZERO!',poly(2)
            ENDIF
            numzerosol = 3
            pdeg = pdeg-1
            IF(abs(poly(3)).le.epsil) THEN
               IF(verb_moid.ge.20) THEN
                  WRITE(*,*)'THIRD DEGREE TERM is CLOSE to ZERO!',poly(3)
               ENDIF
               numzerosol = 4
               pdeg = pdeg-1
...
! ************************************************
   IF(abs(poly(poldg)).le.epsil) THEN
      IF(verb_moid.ge.20) THEN
         WRITE(*,*)'16th DEGREE TERM is CLOSE to ZERO!',poly(poldg)
      ENDIF
      pdeg = pdeg-1
      numzerocoe = 1
      IF(abs(poly(poldg-1)).le.epsil) THEN
         IF(verb_moid.ge.20) THEN
            WRITE(*,*)'15th DEGREE TERM is CLOSE to ZERO!',poly(poldg-1)
         ENDIF
         pdeg = pdeg-1
         numzerocoe = 2
         IF(abs(poly(poldg-2)).le.epsil) THEN
            IF(verb_moid.ge.20) THEN
               WRITE(*,*)'14th DEGREE TERM is CLOSE to ZERO!',poly(poldg-2)
            ENDIF
            pdeg = pdeg-1
            numzerocoe = 3
            IF(abs(poly(poldg-3)).le.epsil) THEN
               IF(verb_moid.ge.20) THEN
                  WRITE(*,*)'13th DEGREE TERM is CLOSE to ZERO!',poly(poldg-3)
               ENDIF
               pdeg = pdeg-1
               numzerocoe = 4
            ENDIF
...
! ***************************
         IF(abs(roots(nroots)).gt.1.d8) THEN
            if(verb_moid.ge.20) then
               write(*,*)'large value for a root',roots(nroots)
            endif
            hzflag=.false.
!            write(*,*)'hzflag',hzflag
         ENDIF
