...
!***********************************************************************
      SUBROUTINE POLZEROS_QP (N, POLY, EPS, BIG, SMALL, NITMAX,            &
     & ROOT, RADIUS, ERR, ITER, APOLY,APOLYR)                           
        IMPLICIT NONE
   INTEGER, PARAMETER :: qkind=kind(1.q0) !for quadruple precision

      INTEGER N,I,NZEROS,ITER,NITMAX 
      COMPLEX(KIND=qkind) :: POLY(N+1),ROOT(N) 
      COMPLEX(KIND=qkind) ::  CORR,ABCORR 
      REAL(KIND=qkind) :: RADIUS(N),APOLY(N+1),APOLYR(N+1) 
      REAL(KIND=qkind) :: AMAX,EPS,SMALL,BIG 
      LOGICAL ERR(N+1) 
...
! Select the starting points                                            
      CALL PZSTART_QP(N,APOLYR,ROOT,RADIUS,NZEROS,SMALL,BIG,ERR) 
...
! Starts Aberth's iterations                                            
      DO 50 ITER=1,NITMAX 
        DO 50 I=1,N 
          IF (ERR(I)) THEN 
            CALL NEWTON_QP(N,POLY,APOLY,APOLYR,ROOT(I),SMALL,RADIUS(I),    &
     &           CORR,ERR(I))                                           
            IF (ERR(I)) THEN 
              CALL ABERTH_QP(N,I,ROOT,ABCORR) 
...
!             .false., otherwise.                                       
!***********************************************************************
      SUBROUTINE NEWTON_QP(N,POLY,APOLY,APOLYR,Z,SMALL,RADIUS,CORR,AGAIN) 
        IMPLICIT NONE
        INTEGER, PARAMETER :: qkind=kind(1.q0) !for quadruple precision
      INTEGER N,I 
      COMPLEX(KIND=qkind) :: POLY(N+1) 
      COMPLEX(KIND=qkind) :: Z,P,P1,CORR,ZI,DEN,PPSP 
      REAL(KIND=qkind) :: RADIUS,APOLY(N+1),APOLYR(N+1) 
      REAL(KIND=qkind) :: AP,AZ,AZI,ABSP 
      REAL(KIND=qkind) :: SMALL 
      LOGICAL AGAIN 
      AZ=ABS(Z) 
...
!***********************************************************************
      SUBROUTINE ABERTH_QP(N,J,ROOT,ABCORR) 
        IMPLICIT NONE
        INTEGER, PARAMETER :: qkind=kind(1.q0) !for quadruple precision
      INTEGER N,I,J 
      COMPLEX(KIND=qkind) :: ABCORR,ZJ 
      COMPLEX(KIND=qkind) :: ROOT(N) 
...
!***********************************************************************
      SUBROUTINE PZSTART_QP(N,A,Y,RADIUS,NZ,SMALL,BIG,H) 
        IMPLICIT NONE
        INTEGER, PARAMETER :: qkind=kind(1.q0) !for quadruple precision
      INTEGER N,NZ,I,IOLD,NZEROS,J,JJ 
      LOGICAL H(N+1) 
      COMPLEX(KIND=qkind) :: Y(N) 
      REAL(KIND=qkind) :: A(N+1),RADIUS(N) 
      REAL(KIND=qkind) :: R,TH,ANG,TEMP,PI2 
      REAL(KIND=qkind) :: SMALL,BIG,XSMALL,XBIG 
!      REAL(KIND=qkind), PARAMETER :: PI2=6.2831853071796q0, SIGMA=0.7q0 
      REAL(KIND=qkind), PARAMETER :: SIGMA=0.7q0 
      XSMALL=LOG(SMALL) 
      XBIG=LOG(BIG) 
      NZ=0 
...
!***********************************************************************
      SUBROUTINE CMERGE_QP(N,A,I,M,H) 
        IMPLICIT NONE
        INTEGER, PARAMETER :: qkind=kind(1.q0) !for quadruple precision
      INTEGER N,M,I,IR,IL,IRR,ILL 
      LOGICAL H(N) 
      LOGICAL TSTL,TSTR,CTEST_QP 
      REAL(KIND=qkind) :: A(N) 
...
! hull of the left and right sets, respectively                         
      CALL LEFT_QP(N,H,I,IL) 
      CALL RIGHT_QP(N,H,I,IR) 
! check the convexity of the angle formed by IL,I,IR                    
      IF (CTEST_QP(N,A,IL,I,IR)) THEN 
        RETURN 
      ELSE 
...
! sets which yield the upper convex hull                                
   ...
   10   CONTINUE 
        IF (IL.EQ.(I-M)) THEN 
          TSTL=.TRUE. 
          ELSE 
          CALL LEFT_QP(N,H,IL,ILL) 
          TSTL=CTEST_QP(N,A,ILL,IL,IR) 
        ENDIF 
        IF (IR.EQ.MIN(N,I+M))THEN 
          TSTR=.TRUE. 
        ELSE 
          CALL RIGHT_QP(N,H,IR,IRR) 
          TSTR=CTEST_QP(N,A,IL,IR,IRR) 
        ENDIF 
        H(IL)=TSTL 
        H(IR)=TSTR 
...
! otherwise CTEST_QP=.FALSE. The parameter TOLER is set to 0.4 by default. 
!***********************************************************************
! Input variables:                                                      
!     N       : length of the vector A                                  
!     A       : vector of double                                        
...
!     .FALSE.,  otherwise.                                              
!***********************************************************************
      FUNCTION CTEST_QP(N,A,IL,I,IR) 
        IMPLICIT NONE
        INTEGER, PARAMETER :: qkind=kind(1.q0) !for quadruple precision
      INTEGER N,I,IL,IR 
      REAL(KIND=qkind) :: A(N) 
      REAL(KIND=qkind) :: S1,S2 
      LOGICAL CTEST_QP 
      REAL(KIND=qkind), PARAMETER :: TOLER=0.4q0 
      S1=A(I)-A(IL) 
      S2=A(IR)-A(I) 
      S1=S1*(IR-I) 
      S2=S2*(I-IL) 
      CTEST_QP=.FALSE. 
      IF(S1.GT.(S2+TOLER)) CTEST_QP=.TRUE. 
      END                                           
