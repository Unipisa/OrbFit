...
c Relativistic perturbations
      double precision drgr(3,7,memx),frel(3,memx),fj2(3,memx)
c Yarkovsky force
      double precision yarkv(21,memx),yarkd(21,memx)
c loop indexes i=1,npla j=1,3
      integer i,j,k,ir,ic,icqfw
c scalar temporaries
      double precision sum,var1
c static memory because of memory of previous somputations
      SAVE
c ===========================================================
...
c general relativistic correction
      if(icrel.eq.1)then
         IF(ips.ne.0) CALL genrel(x,v,drgr(1,1,imem))
         do j=1,3
            f(j)=f(j)+drgr(j,1,imem)
         enddo
      elseif(icrel.eq.2)then
         IF(ips.ne.0) CALL eihrel(x,v,xpla,d,r,rast,frel(1,imem))
         do j=1,3
            f(j)=f(j)+frel(j,imem)
         enddo
c if we need the refined relativity then also include J_2 for the sun
         IF(ips.ne.0) CALL j2sun(x,fj2(1,imem))
         do j=1,3
            f(j)=f(j)+fj2(j,imem)
         enddo
      endif
...
c yarkovsky effect, if required and data are avilable
      if(iyark.ge.1.and.yarfil)then
         ...
         ENDIF
c diurnal
         IF(ips.ne.0) CALL yarkdi(x,yarkd(1,imem),iyarpt)
         do j=1,3
            f(j)=f(j)+yarkd(j,imem)
         enddo
c seasonal
         if(iyark.gt.1)then
            IF(ips.ne.0) CALL yarkse(x,v,yarkv(1,imem),iyarpt)
            do j=1,3
               f(j)=f(j)+yarkv(j,imem)
            enddo
         endif
      endif
c ===========================================================
