...
PROGRAM fil2fla
  ...
  INTEGER :: iunfil,iunfla,i,fail_flag 
...
! *************************************************
  ...
  CALL filopn(iunfil,'vast.fil','old')
  CALL filnam('.','vastfil','fla',elefil,le)  
  CALL filopn(iunfla,elefil(1:le),'unknown')

! Reading the header
  CALL skip(iunfil,1) !skipping asterid name
  CALL reaflc(iunfil,'t0',t0,commen) !reference time and comment 
  tephast0=t0-2400000.5d0
! file header
  WRITE(iunfla,101) tephast0,0.d0,0.d0,0.d0,0.d0,0.d0
  CALL skip(iunfil,2)
...
! Reading evolution from file vast.fil and writing it in file vastfil.fla
  DO i=1,secmax
     READ(iunfil,*,END=333) tevol(i) ! difference of time (yr) from tephast0
     READ(iunfil,100) evolast(i,1:6),ngf(i)
...
101  FORMAT(f10.2,2x,f11.5,2x,f11.5,2x,f10.7,2x,f13.7,2x,f13.7)
  ENDDO
