...
! ***********************************************************       
SUBROUTINE chooserhs(y,om,omnod,g,zl,bigelle,sigma,ddd,eee,nnn) !resonant added elle
  USE fund_const 
  USE orbit_elements
  USE planet_orbits
  USE critical_points
  ...
  IMPLICIT NONE
! ================= INTERFACE ================================ 
  REAL(KIND=dkind),INTENT(IN) :: y(6) !y=[omega,G,Omnod,Z,S,sigma] ! values at previous step
  REAL(KIND=dkind),INTENT(IN) :: om,omnod,g,zl,bigelle,sigma !values at intermediate time steps
!  REAL(KIND=dkind),INTENT(IN) :: aa,elle
  REAL(KIND=dkind),DIMENSION(6),INTENT(OUT) :: ddd,eee
  INTEGER,DIMENSION(6),INTENT(OUT) :: nnn
! ================= END INTERFACE ===========================
  TYPE(orbit_elem) :: elem,elpl !planet and asteroid elems
  REAL(KIND=dkind),DIMENSION(6) :: dd,dd1,ee,ee1,dd_tmp
  INTEGER,DIMENSION(6) :: nn,nn1
!  REAL(KIND=dkind) :: gmp !GM of planets
  REAL(KIND=dkind) :: aa,ea,apl,epl
...
! =================================================================
! *************************************************************     
  aa=(y(5)/ky)**2 !asteroid semimajor axis
...
! Asteroid Keplerian Elements
  ...
  elem%coord(4) = y(3)
  elem%coord(5) = y(1)
  elem%coord(6) = 0.d0 ! has been set to 0 since it is not used here!
...
!    write(*,*)'dmintil values: ',dnodp(i),dnodm(i),i
 ...
 !       write(*,*)'no cross'
      !  CALL rhs2(i,elpl,om,omnod,g,zl,aa,dd,ee,nn)
        CALL rhs2(i,elpl,n_res,om,omnod,g,zl,bigelle,sigma,dd,ee,nn) !resonant
     ELSE
