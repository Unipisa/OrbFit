...
! now y=(omega,G,Omega_nodale,Z,S,sigma) al tempo t+h
! nc deve diventare un indice sui pianeti da 2 a 6                            
! *********************************************************         
SUBROUTINE prsalto(y,elplan,ddG,ddom,ddzl,ddomnod,ddS,nc)
  USE orbit_elements
  USE critical_points
  USE kantorovich, ONLY: coord_tau_Ah,rot_matrices!,elpl,elem
  USE fund_const
  USE right_hand_side,ONLY: kappaa
  IMPLICIT NONE 
! ===================== INTERFACE =======================
  TYPE(orbit_elem),INTENT(IN) :: elplan ! planet orbital elements
  TYPE(orbit_elem) :: elem,elpl !planet and asteroid elems
  REAL(KIND=dkind),INTENT(IN) :: y(6)
  INTEGER,INTENT(IN) :: nc
! ------------------------------------------------------------
  REAL(KIND=dkind),DIMENSION(nminx),INTENT(OUT) :: ddG,ddom,ddzl,ddomnod,ddS
! ======================= END INTERFACE ============================
! Asteroid elements
...
! Varibles for the jump
  ...
  REAL(KIND=dkind),DIMENSION(5) :: ddR ! derivatives pert func jump
! ==================================================================  
! loop indexes                                                      
  INTEGER i,j
! ==================================================================
...
! asteroid keplerian orbital elements
  ...
  elem%coord(1)=aa
  elem%coord(2)=sqrt(1.d0-(y(2)/ky)**2/aa)
  elem%coord(3)=acos(ci)
  elem%coord(4)=y(3)
  elem%coord(5)=y(1)
  elem%coord(6)=0.d0 !DUMMY VALUE (it is not used)
  elem%coo='KEP'
  elem%t=elpl%t
...
! Jacobian matrix dKEP_dDEL ( = d(a,e,I,Omnod,omega)/d(g,z,G,Z,S) )
  dKEP_dDEL=0.d0
  dKEP_dDEL(5,1)=1.d0
  dKEP_dDEL(4,2)=1.d0
  ...
  dKEP_dDEL(1,5)= 2.d0*sqrt(aa)/ky*kappaa
  dKEP_dDEL(2,5)= beta**2/(ky*sqrt(aa)*e)*kappaa
...
! Derivative jump of dmintil w.r. to DEL
     ddmint_del2(1,1:5)= MATMUL(ddmintkep2(1,1:5),dKEP_dDEL(1:5,1:5))          

! Derivative jump of dmin_local w.r. to DEL
     ddmin_del2(1:5)=2*ABS(ddmint_del2(1,1:5))
...
! =========================================================         
!     S DERIVATIVE NORMALIZING JUMP
! =========================================================         
! Derivate jump of perturbing function with respect to S
     ddS(i)=ddR(5)/(dpig**2) 
