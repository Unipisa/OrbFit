...
SUBROUTINE prstop(iunpre,tast0,omfreq,t,y,a,stopfl) 
  USE fund_const
  IMPLICIT NONE
  INTEGER,INTENT(IN) :: iunpre 
  REAL(KIND=dkind),INTENT(IN) :: tast0,omfreq,t,y(4),a
...
! ====================== END INTERFACE =============================
  ...
  INTEGER nsx 
  PARAMETER (nsx=1000) 
  REAL(KIND=dkind) :: om(nsx),e(nsx),omzero,ezero
  REAL(KIND=dkind) :: omnod(nsx),incl(nsx),inclzero,omnodzero
  REAL(KIND=dkind) :: nintomz,chomz 
  REAL(KIND=dkind) :: y1(4),y2(4),targ,yt(4),dt 
...
! eccentricity and inclination at k*pig/2                           
  REAL(KIND=dkind) :: e1,omeg1,e2,omeg2,omnod1,omnod2,incl1,incl2 
...
! transition recorded: regula falsi                                 
                 t1=tim(nprev) 
                 y1(1)=om(nprev) 
                 y1(2)=ky*sqrt(a*(1.d0-e(nprev)**2)) 
                 y1(3)=omnod(nprev)
                 y1(4)=y1(2)*cos(incl(nprev))
                 t2=tim(nst) 
                 y2(1)=om(nst) 
                 y2(2)=ky*sqrt(a*(1.d0-e(nst)**2)) 
                 y2(3)=omnod(nst)
...
! targ=prevsec*pig/2.d0                                             
                 ...
                 CALL falsectra(tast0,t1,y1,t2,y2,omfreq,a,targ,yt,dt)
                 write(*,*)'t1,t2 dopo di falsectra:',t1,t2 
! output                                                            
                 e1=sqrt(1.d0-yt(2)**2/(ky**2*a)) 
                 omeg1=yt(1) 
                 omnod1=yt(3) 
