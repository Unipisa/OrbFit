...
! ====================== selecting derivative =========================   
  IF(ider.eq.1)THEN 
     call dqagsc(loopfg,-pig,pig,epsabs,epsrel,result,abserr,neval, &
          &        ier,limit,lenw,last,iwork,work)                           
!     if(ier.gt.1)write(*,*)'d/G ',ier,u                                
  ELSEIF(ider.eq.2)THEN 
     call dqagsc(loopfom,-pig,pig,epsabs,epsrel,result,abserr,neval, &
          &        ier,limit,lenw,last,iwork,work)                           
!     if(ier.gt.1)write(*,*)'d/dom ',ier,u                              
  ELSEIF(ider.eq.3)THEN 
     call dqagsc(loopfz,-pig,pig,epsabs,epsrel,result,abserr,neval, &
          &        ier,limit,lenw,last,iwork,work)                           
!     if(ier.gt.1)write(*,*)'d/dZ ',ier,u  
  ELSEIF(ider.eq.4)THEN 
     call dqagsc(loopfon,0.d0,dpig,epsabs,epsrel,result,abserr,neval,    &
          &        ier,limit,lenw,last,iwork,work)                           
