...
!  eta = 1.d-7
  ...
  CALL dqagsc(fun,pinf,psup,epsabs,epsrel,result,abserr, &
...
! if(ier.gt.1) write(*,*)'d/G ',ier,f
     ELSE !IF(ABS(u2sgn(1) - u).gt.eta)THEN 
        CALL dqagsc(deltafg,pinf,psup,epsabs,epsrel,result,abserr, &
             & neval,ier,leniw,lenw,last,iwork,work)
! if(ier.gt.1) write(*,*)'d/G ',ier,f
     ENDIF
...
! if(ier.gt.1) write(*,*)'d/dom ',ier,f
     ELSE !IF(ABS(u2sgn(1) - u).ge.eta)THEN
        CALL dqagsc(deltafom,pinf,psup,epsabs,epsrel,result,abserr, &
             & neval,ier,leniw,lenw,last,iwork,work)
! if(ier.gt.1) write(*,*)'d/dom ',ier,f                                
     ENDIF
...
! if(ier.gt.1) write(*,*)'d/dZ ',ier,f
     ELSE !IF(ABS(u2sgn(1) -u).ge.eta)THEN
        CALL dqagsc(deltafz,pinf,psup,epsabs,epsrel,result,abserr, &
             & neval,ier,leniw,lenw,last,iwork,work)
! if(ier.gt.1) write(*,*)'d/dZ ',ier,f
     ENDIF
