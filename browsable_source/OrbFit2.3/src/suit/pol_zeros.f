* Slightly modified by M.Carpino (December 14, 1998): I have only changed
...
*  SMALL, i.e., the greatest and the smallest positive DOUBLE PRECISION numbers,  *
...
*  -- BIG   : the max DOUBLE PRECISION, BIG=2**1023 for the IEEE standard.        *
*  -- SMALL : the min positive DOUBLE PRECISION, SMALL=2**(-1074) for the IEEE.   *
...
C Initialize
      DO 20 I=1,N
        ...
        ERR(I)=.TRUE.
20    CONTINUE
C Select the starting points
      CALL PZSTART(N,APOLYR,ROOT,RADIUS,NZEROS,SMALL,BIG,ERR)
C Compute the coefficients of the backward-error polynomial
      DO 30 I=1,N+1
...
*************************************************************************
      ...
      LOGICAL AGAIN
      AZ=ABS(Z)
C If |z|<=1 then apply Ruffini-Horner's rule for p(z)/p'(z) 
C and for the computation of the inclusion radius
      IF(AZ.LE.1)THEN
        P=POLY(N+1)
        AP=APOLY(N+1)
        P1=P
...
10      CONTINUE
        ...
        RETURN
      ELSE
C If |z|>1 then apply Ruffini-Horner's rule to the reversed polynomial
C and use formula (2) for p(z)/p'(z). Analogously do for the inclusion
C radius.
        ZI=1/Z
        AZI=1/AZ
        P=POLY(1)
...
*************************************************************************
      ...
      COMPLEX*16 ROOT(N)
C The next variable Z could be defined as complex*8 to speed up the
C computation, this slightly reduces the robustness of the program
      COMPLEX*16 Z
      ABCORR=0
      ZJ=ROOT(J)
      DO 10 I=1,J-1
...
************************************************************************* 
      ...
      XBIG=LOG(BIG)
      NZ=0
C Compute the logarithm A(I) of the moduli of the coefficients of
C the polynomial and then the upper covex hull of the set (A(I),I)
      DO 10 I=1,N+1
        IF(A(I).NE.0)THEN
          A(I)=LOG(A(I))
          ELSE
...
10    CONTINUE
      CALL CNVEX(N+1,A,H)
C Given the upper convex hull of the set (A(I),I) compute the moduli
C of the starting approximations by means of Rouche's theorem
      IOLD=1
      TH=PI2/N
      DO 20 I=2,N+1
        IF (H(I)) THEN
...
C Check if the modulus is too small
          ...
          ENDIF
C Check if the modulus is too big
          ...
          IF((TEMP.LE.XBIG).AND.(TEMP.GT.MAX(-XBIG,XSMALL)))THEN
            R=EXP(TEMP)
          ENDIF
C Compute NZEROS approximations equally distributed in the disk of
C radius R
          ANG=PI2/NZEROS
          DO 30 J=IOLD,I-1
            JJ=J-IOLD+1
...
*************************************************************************
      ...
      DO 10 I=1,N
        H(I)=.TRUE.
10    CONTINUE
C compute K such that N-2<=2**K<N-1
      K=INT(LOG(N-2.0D0)/LOG(2.0D0))
      IF(2**(K+1).LE.(N-2)) K=K+1
C For each M=1,2,4,8,...,2**K, consider the NJ pairs of consecutive 
C sets made up by M+1 points having the common vertex
C (JC,A(JC)), where JC=M*(2*J+1)+1 and J=0,...,NJ,
C NJ=MAX(0,INT((N-2-M)/(M+M))).
C Compute the upper convex hull of their union by means of the
C subroutine CMERGE 
      M=1
      DO 20 I=0,K
...
*************************************************************************
      ...
      DOUBLE PRECISION A(N)
C at the left and the right of the common vertex (I,A(I)) determine
C the abscissae IL,IR, of the closest vertices of the upper convex 
C hull of the left and right sets, respectively 
      CALL LEFT(N,H,I,IL)
      CALL RIGHT(N,H,I,IR)
C check the convexity of the angle formed by IL,I,IR
      IF (CTEST(N,A,IL,I,IR)) THEN
        RETURN
      ELSE
C continue the search of a pair of vertices in the left and right
C sets which yield the upper convex hull
        H(I)=.FALSE.
10      CONTINUE
        IF (IL.EQ.(I-M)) THEN
          TSTL=.TRUE.        
